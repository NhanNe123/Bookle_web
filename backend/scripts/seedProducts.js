...
async function seedProducts() {
  try {
    ...
    // Available book images (01.png to 11.png)
    ...
    for (let i = 0; i < sampleProducts.length; i++) {
      ...
      try {
        // Distribute available images across products (cycle through if more products than images)
        ...
        await product.save();
        console.log(`   ✅ Đã tạo: ${product.name} (${product.categories.join(', ')})`);
        created++;
      } catch (error) {
